...
with col_sx:
    ...
    if status == 200:
        ...
        for item in data:
            ...
            # Bitpanda usa balance o amount per le azioni Legacy
            ...
            if qty > 0:
                ...
                with st.container():
                    ...
                    if symbol == "EUR":
                        ...
                        c1.write(f"**{nome}** ({symbol})")
                        c1.caption(f"Posizione: {qty:.4f} unità")
                        
                        # Mostriamo il Valore Totale se abbiamo il Pmc
                        if pmc > 0:
                            valore_attuale = qty * pmc # Approssimazione
                            c2.metric("Valore", f"{valore_attuale:.2f} €", "Live Data")
                            st.caption(f"Prezzo medio carico: {pmc:.2f} €")
                    st.divider()
        
        if not found:
            st.warning("⚠️ Nessun asset trovato. Controlla di aver fatto 'Nuova Versione' su Google Script.")
...
with col_cx:
    ...
    monitorati = ["Leonardo", "Intesa SP", "Amazon", "NVIDIA", "Microsoft"]
    for m in monitorati:
